...
def load_models(data, time_intervals, model_folder= model_folder):
...
def adjust_predictions_direct(test_interval, data, predictions, rf_model, selected_features):
    ...
    ani.save('png/adjustment_animation.mp4', writer='ffmpeg')
    plt.show()
...
def generate_report_with_adjustments(data, predictions, adjusted_predictions, interval):
    ...
    os.makedirs('png', exist_ok=True)
    plt.savefig(f'png/Adjustments_{start}_{end}_1.png')
    plt.show()

# -----------------------------
# Main program
# -----------------------------
with open(os.path.join(model_folder, 'all_predicted_data.txt'), 'r') as f:
...
for interval in time_intervals[1:]:
    ...
    adjusted_predictions = adjust_predictions_direct(test_interval, data, predictions, rf_model, selected_features)
