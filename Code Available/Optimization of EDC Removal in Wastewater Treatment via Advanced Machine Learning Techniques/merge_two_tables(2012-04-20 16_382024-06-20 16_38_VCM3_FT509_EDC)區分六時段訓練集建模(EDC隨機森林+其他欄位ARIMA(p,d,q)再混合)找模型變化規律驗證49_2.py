...
lookback = 1  # 去除頭一筆銜接奇異資料
f = open('png/file_49_2/output.txt', 'w')  # 將print在終端的資料傳到檔案
...
def analyze_intervals(data, intervals, historical_arima_params, historical_varima_params, historical_rf_params):
    ...
    #return models, arima_params_within, varima_params
...
def predict_with_next_params(data, intervals, next_arima_params, next_varima_params, next_rf_model):
    ...
    for column, order in next_arima_params.items():
        ...
        plt.xlabel('Time')
        plt.ylabel('Value')
        plt.savefig(f'png/file_49_2/{column}_Predictions_for_Last_Interval_Predicted_{intervals[-1][0]}_{intervals[-1][1]}.png')
        # plt.show()

    # 預測 EDC 使用 RandomForest
    ...
    plt.xlabel('Time')
    plt.ylabel('EDC')
    plt.savefig(f'png/file_49_2/EDC_Predictions_for_Last_Interval_Predicted_{intervals[-1][0]}_{intervals[-1][1]}.png')
    # plt.show()
...
def evaluate_model(data, test_interval, trained_models, predicted_data):
    ...
    for model in trained_models:
        if model['type'] == 'ARIMA':
            ...
            plt.ylabel('Value')
            plt.savefig(
                f'png/file_49_2/{column}_Model_Evaluation_for_Test_Interval_{test_interval[0]}_{test_interval[1]}.png')
            # plt.show()
            ...
            plt.xlabel('Time')
            plt.ylabel('EDC')
            plt.savefig(f'png/file_49_2/EDC_Model_Evaluation_for_Test_Interval_{test_interval[0]}_{test_interval[1]}.png')
            # plt.show()

# 保存結果到 CSV
def save_results_to_csv(models, arima_params_within, varima_params):
    models_df = pd.DataFrame(models)
    models_df.to_csv('png/file_49_2/models.csv', index=False)
    ...
    arima_params_df.to_csv('png/file_49_2/arima_params.csv', index=False)
    ...
    varima_params_df.to_csv('png/file_49_2/varima_params.csv', index=False)
        ...
        for model in models:
            if model['interval'] == (start, end):
                if model['type'] == 'ARIMA':
                    ...
                    plt.xlabel('Time')
                    plt.ylabel('Value')
                    plt.savefig(f'png/file_49_2/{column}_Predictions_for_Interval_{start}_to_{end}_file.png')
                    # plt.show()
                elif model['type'] == 'RandomForest':
                    plt.figure(figsize=(14, 7))
                    edc_data = interval_data.dropna(subset=['EDC'])
                    ...
                    plt.xlabel('Time')
                    plt.ylabel('EDC')
                    plt.savefig(f'png/file_49_2/EDC_Predictions_for_Interval_{start}_to_{end}_file.png')
                    # plt.show()
...
def plot_combined_results(models, test_intervals, all_predicted_data):
    for idx, (start, end) in enumerate(test_intervals):
        ...
        plt.xlabel('Time')
        plt.ylabel('Value')
        plt.savefig(f'png/file_49_2/Combined_Results_for_Interval_{start}_to_{end}.png')
        # plt.show()
