...
def run_qiskit_aer():
    ...
    for p in ps:
        ...
        noise_model.add_all_qubit_quantum_error(err1, ['rz','h'])
        noise_model.add_all_qubit_quantum_error(err2, ['cp'])

        qc_qft = qft3_circ()
        qc_qba = qba_like_circ()
        qc_qft.save_density_matrix()
        qc_qba.save_density_matrix()

        # random pure input is more complex to inject in Aer with noise repeatedly; here compare ideal vs noisy fidelity via density matrices
        # so run ideal on statevector simulator first to get |psi_id>
        psi0 = Statevector.from_int(0, 2**3)  # fixed input |000> for simplicity
        psi_id_qft = psi0.evolve(qc_qft)
        psi_id_qba = psi0.evolve(qc_qba)

        rho_id_qft = density_matrix(psi_id_qft)
        rho_id_qba = density_matrix(psi_id_qba)

        noisy_qft = backend.run(qc_qft, noise_model=noise_model).result().data(0)['density_matrix']
        noisy_qba = backend.run(qc_qba, noise_model=noise_model).result().data(0)['density_matrix']

        # Fidelity for pure vs mixed reduces to <psi_id|rho|psi_id>
        F_qft = np.real(psi_id_qft.data.conj().T @ noisy_qft @ psi_id_qft.data)
        F_qba = np.real(psi_id_qba.data.conj().T @ noisy_qba @ psi_id_qba.data)
        rows.append({'p_depol':float(p), 'F_QFT':float(F_qft), 'F_QBA_like':float(F_qba)})
