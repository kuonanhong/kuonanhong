...
def geturl(url):                                          #這函式是要自動化取得urllist內每個網址
    ...
    headers['Referer'] = urlform + '/Tourism-'+ urlele.split('-')[1] + '-'+ urlele.split('-')[2] + '-'+ urlele.split('-')[3]
...
#            reviewPageDic[data_page_number.format(reviewPageNumber)] = reviewDic     #將此地區這個特定餐廳第一頁資訊塞成reviewPageDic[data_page_number n]
            ...
            print ('!!!!!!!!!!!!!!!!!!!!!!!!')
        RestantDic[soup2.select('.listing_title')[0].select('.property_title')[0].text] = EachDayCommentDic
...
#    print (tempDic['area'] + u'這地區總共有'+ ''.join(m.group(1).split(',')) +u'家精選的旅行社,為妳精選其中幾家的評論')
    print (tempDic['area'] + u'這地區總共有1020家精選的旅行社,為妳精選其中幾家的評論')
    countRest = 0
...
#    while(countRest < int(''.join(m.group(1).split(',')))/10):      #在網路不斷線下, 預計抓其中10分之1的餐廳量(不重複)需1天(p.s.因為跳出餐廳之頁面是亂數跳出)
    while(countRest < 1020/10):                                     #北海道地區沒有soup.select('.tab_count')的標籤,直接蟲網路看最後一頁是1020家餐廳
        soup2 = geturlpost(urlHotels)
#            RestantDic[hotel.format(countRest+1)] = soup2.select('.listing_title')[0].select('.property_title')[0].text
#            if (RestantDic[hotel.format(countRest+1)] in List):
        if (soup2.select('.listing_title')[0].select('.property_title')[0].text in List):
            continue
        else:
            ...
            List.append(soup2.select('.listing_title')[0].select('.property_title')[0].text)

        print ('--------------------------------------------------------------------------')
    print tempDic['area'] +u'的區域所有餐廳評論結束'
    tempDic['hotel_comments'] = RestantDic
...
import json
with open('TripAdvisor飯店評論北海道地區精簡正式版.json', 'w') as f:                               #轉成.json檔
