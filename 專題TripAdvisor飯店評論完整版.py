...
projectList = []
urlform = 'https://www.tripadvisor.com.tw'         #下面url所共有的表頭網址,每個在urllist上的網址代表一個套裝行程(大興旅行社日本行)
...
#    tempDic['hotel'] = soup2.select('.listing_title')[0].select('.property_title')[0].text
    ...
    print RestantDic['hotel']                                     #印出tempDic['hotel']的值
...
#            if (tempDic['hotel'] not in List):                           #看動態更新的網頁其跳出的餐廳有無和之前重複
            ...
            List.append(tempDic['hotel'])                                       #這一步藉由copy的動做新建一個物件,以防止相同key值其value植被覆蓋
            print tempDic['hotel'] + u'此餐廳的所有評論結束'
            print ('--------------------------------------------------------------------------')
