...
num = 'num{}'
count = 1
with io.open('TripAdvistor東京京都大阪神戶觀光活動細項目條列與旅遊評論字典檔.txt', 'w',encoding='UTF-8') as f:       #將資料輸入檔案
    ...
    #        tempDic['觀光景點'+num.format(countpoint)] = ele.text
            ...
            f.write('\n'+tourDic['觀光景點']+u'的景觀類型是: '+tourDic['景點類型']+'\n')        #把這個景點寫入.txt之中
            f.write(u'============================================================')
...
#                tempDic[num.format(countpoint)] = tourDic['景點評論']         #包含了getReviews內EachDayCommentDic['觀光景點與景點類型']當成disc的第一項與其他分項代表旅客不同時間點的評論
        ...
        print('=========================================================')
        placeDic[urlele.split('-')[4].split('_')[0]] = tempDic                             #把整個關關景點再塞入日本地區, 如:東京,京都等
f.close()
...
with open('TripAdvistor東京京都大阪神戶觀光活動細項目條列與旅遊評論字典檔.json', 'w') as f:                               #轉成.json檔
