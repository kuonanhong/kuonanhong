...
urllist = ['/Attractions-g298184-Activities-c47-Tokyo_Tokyo_Prefecture_Kanto.html'                #東京都
#           '/Attractions-g298184-Activities-c47-oa30-Tokyo_Tokyo_Prefecture_Kanto.html#ATTRACTION_LIST',
#           '/Attractions-g298184-Activities-c47-oa1200-Tokyo_Tokyo_Prefecture_Kanto.html#ATTRACTION_LIST'
           ]
tempDic ={'觀光景點':''}
...
for urlele in urllist:
    ...
    for ele in soup.select('.wrap.al_border.attraction_element'):
        tempDic['觀光景點'] = ele.text
        print ele.select('.entry')[0].select('.property_title')[0].text
        ...
        soupReview = geturl(urlform + ele.select('a')[0]['href'])                  #先送入geturl以得到後續爬蟲處理
        print soupReview.select('.innerBubble')[0].text
#        tourDic['景點評論'] =
...
with open('TripAdvistor東京都觀光活動細項條列.json', 'w') as f:                               #轉成.json檔
